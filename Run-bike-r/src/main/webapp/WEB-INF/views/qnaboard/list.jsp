...
<script>
    ...
    function editPreSet(q_num){
        ...
        $('#editFrame').css('display', 'block');
           ...
           $.ajax({
                url : 'http://15.164.162.21:8080/runbike/rest/board/'+q_num,
                type : 'GET',
  ...
  //수정하기(글번호를 찾아서 수정)
	  ...
	  function list(pgNum){
			...
			$.ajax({
				url : 'http://15.164.162.21:8080/runbike/rest/board/list',
				type : 'GET',
			...
			//답글 작성폼
 		    function replywrite(q_num){
 		       ...
 		       /*  html += '<input type="hidden" value="'+u_idx+'" id="u_idx" name="u_idx">'; */
 		        ...
 		        html += '<div class="form-group col-lg-3"><label for="rp_writer">작성자</label><textarea class="form-control"  placeholder="관리자" id="rp_writer" name="rp_writer" col="100" required></textarea></div>';
			 ...
			 function submitForm(q_num){				 
			        ...
			        $.ajax({
			            type : 'POST',
			            url : 'http://15.164.162.21:8080/runbike/rest/reply',
			...
			function getreplylist(q_num){
			    	...
			    	$.ajax({
						url : 'http://15.164.162.21:8080/runbike/rest/reply/reply/'+q_num,
						type : 'GET',
						success : function(data){
							if(data.length>0){
...
// 				}
			...
			function detaildata(q_num){
				...
				$('#detailFrame').css('display','block');
					...
					$.ajax({
						url : 'http://15.164.162.21:8080/runbike/rest/board/detail/'+q_num,
						type : 'GET',
						 success : function(data){
		  ...
		  //문의글삭제(원글 글 번호로)
		    function del(q_num){
		        ...
		        if(confirm('정말 삭제하시겠습니까?')){
		           $.ajax({
		                url : 'http://15.164.162.21:8080/runbike/rest/board/del/'+q_num,
		                type : 'DELETE',
		                success : function(data){
		                        if(data=='SUCCESS'){
		                        alert('삭제가 완료되었습니다');
		...
		function delreply(rp_num){
			...
			if(confirm('정말 삭제하시겠습니까?')){
		           $.ajax({
		                url : 'http://15.164.162.21:8080/runbike/rest/reply/del/'+rp_num,
		                type : 'DELETE',
		                success : function(data){
