...
<!-- 푸터 끝 -->
    ...
    <script type="text/javascript">
	    ...
	    /* 방을 만들 때 현재시간을 자동으로 넣어주기 위해, 타임존을 반영한 한국의 현재시각을 구한다 */
	    ...
	    /* 0. 방 출력 및 참여 관련 함수들 시작 ======================================================== */
	    
		/* party 테이블에 저장된 방을 뿌려준다 */
		function list() {
			$.ajax({
				url : 'party/list',
				type : 'GET',
		...
		/* 방에 참여하기 전, 방의 인원이 꽉 차진 않았는지, 비밀번호가 틀리진 않았는지 검사한다 */
        ...
        /* 방 만들기에 사용할 지도 초기화 함수 */
        function initTmap() {
    
			// (반응형) 일단은 이렇게 해놓았는데, 다른 방법이 필요해보인다.
		...
		/* 마커를 선택하면 팝업을 띄워주는 함수, POI 상세 정보 API 사용 */
        function poiDetail(poiId, se) { 
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // 시작지를 검색했으면 시작지 선택이, 도착지를 검색했으면 도착지 선택이 나타나게 한다.
                    if (se == 'S') {
